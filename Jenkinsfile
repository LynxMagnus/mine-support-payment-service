...
node {
  ...
  try {
    ...
    stage('Create database role and schema') {
      ...
      defraUtils.destroyPrRoleAndSchema(host, dbname, credentialsId, pr, true)
      (prSchema, prUser) = defraUtils.provisionPrRoleAndSchema(host, dbname, credentialsId, prCredId, pr, false)
